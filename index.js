...
async function sendDynamicInvite(group_chat_id, chat_id_or_user_id) {
  ...
  try {
    ...
    if (resp?.ok && resp?.result?.invite_link) {
      ...
      console.error("InviteLink API resp:", resp);
    }
  } catch (e) {
...
// ──────────────────────────────────────────────────────────────────────────────
// Middlewares (Stripe-Webhook braucht RAW, Telegram normal JSON)
// ──────────────────────────────────────────────────────────────────────────────
...
// Express-Handler für Telegram-Updates (sichtbares Logging)
...
async function bootstrapTelegram() {
  try {
    // Bestehenden Webhook sicher löschen, dann frisch setzen
    ...
    const setResp = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/setWebhook`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: telegramWebhook }),
    }).then((r) => r.json());
    ...
    if (msg.chat.type === "group" || msg.chat.type === "supergroup") {
      if (!creator_id) {
        await bot.sendMessage(
          msg.chat.id,
          "❓ Kein Creator-Payload. Nutze den „Gruppe verbinden“-Button in den Einstellungen.\nTipp: In Gruppen ggf. `/start@DeinBotName creator_<id>` verwenden.",
          { parse_mode: "Markdown" }
        );
        return;
      }
  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      const { data: vipRow } = await supabase
        .from("vip_users")
        .upsert({ creator_id, telegram_id, chat_id, status: "aktiv", vip_bis }, { onConflict: "creator_id,telegram_id" })
